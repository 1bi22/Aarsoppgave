...
let tetrisScore = 0; // Tracks the score
...
function SetupCanvas(){
    ...
    tetrisBoard.width = 936;
    tetrisBoard.height = 956;
    ...
    ctx.fillStyle = 'white';
    ctx.fillRect(0, 0, tetrisBoard.width, tetrisBoard.height);
    ...
    //Lag intstruks for hvordan starte spill
    
    // Draw score rectangle
    ctx.strokeRect(300, 107, 161, 24);
    
    // Draw score
    ctx.fillText(tetrisScore.toString(), 310, 127);
...
// Define the function to be executed at the interval
...
// Function to start the interval when spacebar is pressed
function startIntervalOnSpacebar(event) {
    if (event.keyCode === 32) { // 32 is the keycode for spacebar
        intervalId = window.setInterval(moveTetromino, 1000);
        window.removeEventListener('keydown', startIntervalOnSpacebar); // Remove the event listener after starting the interval
    }
}

// Add event listener for keydown events
window.addEventListener('keydown', startIntervalOnSpacebar);
